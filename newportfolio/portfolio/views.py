...
class IndexView(View):
    ...
    def get(self, request):
        ...
        PI.info = PI.info.replace("{{visitor_location}}", visitor_location) #Replaces the {{visitor_location}} in admin
        req_headers = ""
        req_META = ""
        ...
        ctx = {'back': back, 'PI': PI, 'cv':cv, 'swiperstack':swiperstack, 'footer':footer, 'groups':groups, 'careerInfo': careerInfo, 'visitor_location':visitor_location}
...
class ProjectsView(View):
    ...
    def get(self, request):
        ...
        ctx = {'back': back, 'footer':footer, 'projects':projects}
        return render(request, self.template, ctx)